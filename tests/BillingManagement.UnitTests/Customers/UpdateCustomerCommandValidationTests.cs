...
public sealed class UpdateCustomerCommandValidationTests
{
    private readonly AnnotationCommandValidator<UpdateCustomerCommand> validator = new();
    ...
    public void Validate_returns_required_and_field_length_errors()
    {
        ...
        Assert.Equal(["Customer name is required."], errors[nameof(UpdateCustomerCommand.CustomerName)]);
        Assert.Equal(["Must not exceed 100 characters."], errors[nameof(UpdateCustomerCommand.TaxId)]);
        Assert.Equal(["Must not exceed 2000 characters."], errors[nameof(UpdateCustomerCommand.Notes)]);
    }
    ...
    public void Validate_rejects_invalid_email(string email)
    {
        ...
        Assert.Equal(["Email format is invalid."], errors[nameof(UpdateCustomerCommand.Email)]);
    }
